...
module Skylab::GitViz::TestSupport
  ...
  describe "[gv] VCS adapters - git - models - bundle - magnetics - matrix [..]" do
    ...
    it "the matrix box's keys are in order" do
      ...
      _actual = _matrix.order_box.a_.map do |long_SHA|
        long_SHA[ 0, at_( :SHORT_SHA_LENGTH_ ) ]
      end

      expect( _actual ).to eql %w(
        fafa001
        fafa002
        fafa003
        fafa004
    ...
    it "the matrix box's value are column offsets" do
      ...
      expect( bx.fetch "fafa001000000000000000000000000000000000" ).to be_zero
      expect( bx.fetch "fafa005000000000000000000000000000000000" ).to eql 4
    end

    it "the matrix has the bundle" do

      expect( _matrix.bundle ).to be_respond_to :ci_box
    end
