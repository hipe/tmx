...
module Skylab::Flex2Treetop
  ...
  class Core::Client
    ...
    Headless::Client[ self, :client_services, :parameter_controller ]  # [#hl-092] guaranteed to change

    client_services_class
    class Client_Services
      delegators :actual_parameters, :builder, :translate_name, :info
      def params
        client.actual_parameters
      end
    end

    Headless::SubClient::Define_emitters_curriable[ :public, [ :info ], self ]
    ...
    def builder
      @builder ||= TreetopBuilder.new core_paystream
    end                           # a service to sub-clients
  ...
  module API
    ...
    def self.invoke method, actuals_h=nil  # #pattern [#sl-121] facade
      api_client_instance.invoke method, actuals_h
    end

    def self.translate actuals_h  # pattern [#sl-121] facade
      api_client_instance.invoke :translate, actuals_h
    end

    class << self
      private
      def api_client_instance
        @api_client_instance ||= API::Client.new nil, nil,
          Headless::System.system.some_stderr_stream
      end
    end
  end
  ...
  class API::Client < Core::Client
    ...
    Headless::API::Client[ self, :xyzzy ]

    client_services_class

    class Client_Services
      delegators :emit_notify
    end

    def initialize( * _ )
      @expression_agent = API::Expression_Svcs__.new
      super
    end
    ...
    def translate request_h=nil
      ...
      begin
        ...
        begin
          params.result_state = Translation.new( client_services ).invoke
        ensure
          @paystream.closed? or @paystream.close
          @upstream.closed? or @upstream.close
        end
        o = params.result_state
      end while false
      o
  ...
  private
    ...
    def emit i, x
      case i
      when :payload ; @payload_x_a << x
      when :error   ; @error_x_a << x
                    ; @y << "(api #{ i } preview): #{ x }"
      else          ; @y << "(api #{ i }): #{ x }"
      end
      nil
    ...
    def resolve_io
      ...
      if ! p.result_state
        change_upstream_notify params.flexfile.open 'r'
        change_paystream_notify params.outfile.open 'w'
      end
      ! p.result_state
    end
    ...
    def some_valid_action_name_a
      @some_valid_action_names_a ||= build_valid_action_names
    end

    def build_valid_action_names
      require 'set' ; add_p = (( set = ::Set.new )).method :<<  # meh
      mod = (( scn = build_modules_scanner ))[]
      begin
        mod.public_instance_methods( false ).each( & add_p )
      end while (( mod = scn[] ))
      set.to_a
    end

    def build_modules_scanner
      mod = self.class
      -> do
        mod or break
        r = mod
        mod = mod.const_defined?( :F2TT_ACTIONS_MODULE_NEXT__, false ) &&
          mod.const_get( :F2TT_ACTIONS_MODULE_NEXT__, false )
        r
      end
    end

    F2TT_ACTIONS_MODULE_NEXT__ = Flex2Treetop::Core::Client
  end

  class API::Expression_Svcs__
    include Headless::API::Expression::Service_Methods__
  end
  ...
  class CLI::Client < Core::Client
    ...
    Headless::CLI::Client[ self, :three_streams,
      :instance_methods, :some_expression_agent, :resolve_upstream ]  # [#hl-092] guaranteed to change / was [#008]
  ...
  private
    ...
    def show_tests
      require 'fileutils'
      pwd = ::Pathname.pwd
      emit :info, say { "#{ kbd 'some nerks to try:' }" }
    ...
    def translate flexfile
      r = resolve_instream_from_upstream
      if r
        r = Translation.new( client_services ).invoke
      end
      if false == r
  ...
  Sub_Client_ = Headless::SubClient::Module.new # :delegators, :

  module Sub_Client_
  private

    def emit i, x
      @client.emit_notify i, x
      nil
    end
  end
  ...
  class Translation
    ...
    def initialize client
      ...
      @instream, @outstream, @errstream = client.three_streams
    ...
    def invoke
      ...
      if ! result
        ...
        require 'pp'
        ::PP.pp result.sexp, @errstream
        :showed_sexp
      elsif result.sexp.translate client
        :translated
      else
        :translate_failure
      end
