...
module Skylab::Flex2Treetop
  ...
  class CLI::Client
    ...
    Headless::CLI::Client[ self, :client_instance_methods,
  ...
  module API
    ...
    private
      def when_infile_not_exist e
        emit :no, :infile_not_found do e end
      end
      def when_infile_exist
        if FILE_FTYPE_ == @infile_stat.ftype
          when_infile_is_file
        else
          emit :no, :infile_is_not_file do @infile_stat end
        end
      end
      ...
      def when_outfile_not_exist
        emit :yes, :when_outfile_not_exist do @out_pn end
      end
      def when_outfile_exist
        if FILE_FTYPE_ == @outfile_stat.ftype
          when_outfile_is_file
        else
          emit :no, :outfile_is_not_file do @outfile_stat end
        end
      end
      def when_outfile_is_file
        if @outfile_stat.size.zero?
          emit :yes, :when_outfile_empty do @out_pn end
        else
      ...
      def when_outfile_nonzero_length
        if @force_is_present
          ...
          emit :no, :outfile_exists_and_force_is_not_present do @out_pn end
        end
      end
      def when_force
        @out_pn.open( 'r' ) do |fh| @first_line = fh.gets end
        if AUTOGENERATED_RX =~ @first_line
          emit :yes, :when_overwriting_non_empty_file do @out_pn end
        else
          emit :no, :outfile_does_not_appear_generated do @first_line end
        end
      end
      def emit * i_a, &p
        @listener.call( * i_a, & p )
  ...
  Class_as_function__ = -> p do  # #storypoint-415
    the_class = MetaHell::FUN::Memoize[ p ]
    -> * x_a do
  ...
  private
    ...
    def autogenerated_line
      AUTOGENNED_TEMPLATE__.
          gsub Headless::Services::Basic::String::MUSTACHE_RX do
  ...
  module Node_Module_Methods__
    ...
    def at_compile str
      res = []
      s = Headless::Services::StringScanner.new(str)
      begin
        if s.scan(/\*/)
          res.push '*'
        elsif s.scan(/\[/)
  ...
  Svcs__::CommonNode = MetaHell::FUN::Memoize[ -> do
  ...
  Svcs__::Treetop_Builder = MetaHell::FUN::Memoize[ -> do
