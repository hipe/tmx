...
## introduction to model components (interspersed with pseudocode)
  ...
  • "boolean trees" (to be expanded upon below) will be specfic kinds
    ...
    to "AND-together" (or "OR-together") two FES's, they must be subsets
    of the same entity model (in this case, the counting numbers).
  ...
  • a "model adapter" will associate one (and for now only one)
    ...
    particular platform class in the particular clienet application is
