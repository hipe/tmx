...
/*
 ...
 /* # "constants" (and defaults for possible future options)
    ...
    [codepen example]:http://codepen.io/osublake/pen/RNLdpz]
 ...
 */
 ...
 /* # section 3 - reordering
    ...
    3) from top to bottom of each displaced piece, calculate a new top with
       simple arithmetic, taking into account gutters somehow.
  */
  ...
  */
  ...
  function __animationMethods( o ) {
    ...
    o.whenPress = function() {
      ...
      var _tw = new TweenLite( el, __moment, {
        autoAlpha: 0.75,
        boxShadow: __shadow2,
        paused: true,
        scale : 0.95,
    ...
    o.snapToIntendedLocation = function() {
     ...
     /* the converse of the above method. the item has stopped moving now.
        ...
        EEK (note1)[] since the tile being dragged is probably "shrunken"
        (has a scale tranform on it), jQuery's `position()` method will
        (reasonably) take this scale into account when calculating the
        `position()`. (the "top" of a shrunken element will be a larger Y
        value than if the element were full-size, all other aspects being
        equal.) however, we are transforming it back to normal size as we
        move it. hence we don't want the scale tranform of the element to
        interfere with us getting a "pure" reading of this imaginary normal
        top hence we can't use jQuery's `position()` method. SO:
      ...
      */
      ...
      if ( false === orig_Y_transform ) {
        ...
        var _tw = new TweenLite( this.element(), waltz, {
          autoAlpha: 1, // 0.75,
    ...
    o.whenNewTop = function() {
     ...
     /* this default implementation is intended for those tiles that are
        ...
        in the middle of an existing animation when this signal is received.
     ...
     */
      ...
      var _s = relativePixelsStringViaDelta( delta );
   ...
   /* ### parse our own CSS :(
      ...
      see note [here](note1) (TODO: footnotes in markdown?)
    */
  ...
  */
  ...
  function __catalogItems( results, movingTile, scrubber ) {
    ...
    function __addMovingItem() {
      ...
      var __scrubberVertCenter = __calculateScrubberVertCenter( item );
      item.scrubberCenterY = function() {
        return item.cachedTop + __scrubberVertCenter;
      };
    ...
    function __calculateScrubberVertCenter( item ) {

      // WARNING - what "top" means is very CSS dependant!

      var _localTop = scrubber.position().top - item.cachedTop;
