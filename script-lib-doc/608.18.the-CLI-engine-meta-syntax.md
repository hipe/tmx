...
(todo XX: there's a lot of high-level documentation in the "engine" test
file, whose writing predates the birth of this document. It should be melted
in to here.)
...
lower-level concepts, we won't onsider it explicitly after this point; but keep
...
practical distillation (which is not really one table but two (1x2x2 + 1x2)):
      ...
      posiitonal optional one        nested_optional_positionals
...
`nonpositionals` before anything else (except subcommands): Purusant to the
...
Having written a great many argument parsers "by hand" that manage parsing
for one or more positional parameters (at least two or three (gasp!)), we have
